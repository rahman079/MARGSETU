 ...
 * @param {boolean} alternatives - Request alternative bypass routes
 */
 ...
 * Sample locations payload: { "locations": [{ "latitude": 25.8241, "longitude": 93.7845 }, ...] }
...
export async function fetchElevationProfile(coordinates) {
  ...
  // Sample up to 10 evenly spaced points along the corridor to avoid payload overload
  ...
  try {
    ...
    console.debug('[Elevation API] Live lookup fallback to GIS elevation model:', err.message);
  }

  // Graceful deterministic fallback based on North-Eastern terrain coordinates
